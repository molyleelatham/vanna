...
Fallback policy: ``*_if_live`` returns None whenever live data is unavailable
(so callers can skip blending), while ``*_or_fallback`` returns fixed constants
for tool-loop answers. Fallback constants are never blended into assessments.
"""
...
class ConnectorClient:
    ...
    # --- Live-or-None accessors ---
    # These catch ANY connector failure (including non-ConnectorError escapes
    # from the Alpha Vantage path) and return None so callers can skip blending
    # instead of diluting assessments with fixed constants.
    def market_data_if_live(self, pair: str, window: str = "1h") -> MarketData | None:
        try:
            return self.market_data(pair, window)
        except Exception:
            return None

    def order_flow_if_live(self, provider: str, window: str = "24h") -> OrderFlowStats | None:
        try:
            return self.order_flow(provider, window)
        except Exception:
            return None

    def execution_history_if_live(
    ...
    def risk_metrics_if_live(self, pair: str) -> RiskMetrics | None:
        try:
            return self.risk_metrics(pair)
        except Exception:
            return None

    def surveillance_signal_if_live(self, provider: str, window: str = "24h") -> SurveillanceSignal | None:
        try:
            return self.surveillance_signal(provider, window)
        except Exception:
            return None

    def federation_metrics_if_live(self) -> FederationMetrics | None:
        try:
            return self.federation_metrics()
        except Exception:
            return None

    # --- Fallback Helpers (tool loop only; never blended into assessments) ---
    def market_data_or_fallback(self, pair: str, window: str = "1h") -> MarketData:
        live = self.market_data_if_live(pair, window)
        if live is not None:
            return live
    ...
    def order_flow_or_fallback(self, provider: str, window: str = "24h") -> OrderFlowStats:
        live = self.order_flow_if_live(provider, window)
        if live is not None:
            return live
        return OrderFlowStats(
    ...
    def execution_history_or_fallback(
        ...
        live = self.execution_history_if_live(provider, pair, size_bucket, window)
        if live is not None:
            return live
        return ExecutionHistory(
    ...
    def risk_metrics_or_fallback(self, pair: str) -> RiskMetrics:
        live = self.risk_metrics_if_live(pair)
        if live is not None:
            return live
        return RiskMetrics(
    ...
    def surveillance_signal_or_fallback(self, provider: str, window: str = "24h") -> SurveillanceSignal:
        live = self.surveillance_signal_if_live(provider, window)
        if live is not None:
            return live
        return SurveillanceSignal(
    ...
    def federation_metrics_or_fallback(self) -> FederationMetrics:
        live = self.federation_metrics_if_live()
        if live is not None:
            return live
        return FederationMetrics(
